<template>
    <div id="form-devices" class="flex flex-col p-6 bg-white w-2/3 shadow-xl rounded-2xl">
        <p class="mb-6 text-lg text-primary-200 font-semibold bg-primary-25 -m-5 px-6 py-3 rounded-t-xl">
           {{ company == null ? 'Crear Dispositivo:' :' Editar Dispositivo:' }}
        </p>
        <el-form :model="ruleForm" :rules="rules" :ref="nameRef">
            <div class="w-full flex flex-col lg:flex-row">
                <el-form-item prop="nameDevice" class="w-full lg:w-1/2 lg:mr-2">
                    <div class="text-sm text-gray-900 font-semibold mb-2">
                        ...
                        <span v-if="company == null" class="text-tertiary-200">*</span> 
                    </div>
                    <el-input v-model="ruleForm.nameDevice" placeholder="Nombre del dispositivo" />
                </el-form-item>
            ...
            <div class="w-full flex flex-col lg:flex-row">
                <el-form-item prop="typeDevice" class="w-full lg:w-1/2 lg:mr-2">
                    ...
                    <el-select v-model="ruleForm.typeDevice" filterable placeholder="tipo de dispositivo"
                        style="width: 100%">
                        <el-option v-for="item in typeDeviceOptions" :key="item.value" :label="item.label"
                            :value="item.value" />
                    </el-select>
                </el-form-item>
                <el-form-item prop="typeService" class="w-full lg:w-1/2 lg:ml-2">
                    ...
                    <el-select v-model="ruleForm.typeService" placeholder="Selecciona el tipo de servicio"
                        style="width: 100%">
                        <el-option v-for="travel in typeServiceOptions" :key="travel.value" :label="travel.label"
                            :value="travel.value" />
                    </el-select>
                </el-form-item>
            </div>
            <div class="w-full">
                ...
                <el-form-item prop="userId" class="w-full" v-if="company == null">
                    <el-select v-model="ruleForm.userId" filterable placeholder="Usuario" style="width: 100%">
            ...
            <div class="w-full pt-4">
                <el-form-item prop="enabled" class="w-full">
                    ...
                    <el-switch v-model="ruleForm.defaultDevice" inline-prompt active-text="Si" inactive-text="No" />
                </el-form-item>
            </div>
            <hr class="mt-2 mb-6 border-tertiary-100">
            <div class="w-full flex flex-col lg:flex-row" v-if="ruleForm.typeDevice == 'Vehiculo'">
                <el-form-item prop="typeVehiclerefilter" class="w-full lg:w-1/2 lg:mr-2">
                    <div class="text-sm text-gray-900 font-semibold my-2 mr-4">
                        Tipo de vehículo
                        <span v-if="company == null" class="text-tertiary-200">*</span> 
                    </div>
                    <el-select v-model="typeVehiclerefilter" filterable placeholder="tipo de vehiculo" style="width: 100%">
                        <el-option v-for="item in typeOptions" :key="item.value" :label="item.label" :value="item.value" />
                    </el-select>
                </el-form-item>
                <el-form-item prop="classesVehiclerefilter" class="w-full lg:w-1/2 lg:ml-2">
                    <div class="text-sm text-gray-900 font-semibold my-2 mr-4">
                        Clase de vehículo
                        <span v-if="company == null" class="text-tertiary-200">*</span> 
                    </div>
                    <el-select v-model="classesVehiclerefilter" placeholder="Selecciona una clase" style="width: 100%">
                        <el-option v-for="travel in classesOptions" :key="travel.value" :label="travel.label"
                            :value="travel.value" />
                    </el-select>
                </el-form-item>
            </div>
            <div class="w-full flex flex-col lg:flex-row">
                <el-form-item prop="typeVehicle" class="w-full "
                    v-if="this.typeVehiclerefilter != '' && this.classesVehiclerefilter != ''">
                    <div class="text-sm text-gray-900 font-semibold my-2 mr-4">
                        Vehículo
                        <span v-if="company == null" class="text-tertiary-200">*</span> 
                    </div>
                    <el-select v-model="ruleForm.typeVehicle" filterable placeholder="vehiculo" style="width: 100%">
                        <el-option v-for="item in VehiclesOptions" :key="item.value" :label="item.label"
                            :value="item.value" />
                    </el-select>
                </el-form-item>
            </div>
...
export default defineComponent({
    ...
    data() {
        ...
        return {
            ...
            VehiclesOptions: [],
            usersOptions: [],
            typeDeviceOptions: [{ label: 'Diadema', value: 'Diadema' }, { label: 'Vehiculo', value: 'Vehiculo' }, { label: 'Dispositivo distracciones', value: 'Dispositivo distracciones' }, { label: 'Botón de pánico', value: 'Botón de pánico' },],
            typeServiceOptions: [{ label: 'Privado', value: 'Privado' }, { label: 'Público', value: 'Publico' }],
            rules: {
                nameDevice: [
                    {
                        required: true,
    ...
    watch: {
        classesVehiclerefilter: {
            handler() {
                if (this.typeVehiclerefilter != '' && this.classesVehiclerefilter != '') {
                    this.getVehicles()
                }
            },
            immediate: false
    ...
    methods: {
        ...
        getVehicles() {
            device.getVehicles(this.typeVehiclerefilter, this.classesVehiclerefilter).then((result) => {
            ...
            this.$refs[this.nameRef].validate((valid) => {
                if (valid) {
                    ...
                    var data = {
                        ...
                        id_tipo_vehiculo: this.ruleForm.typeVehicle != '' ? this.ruleForm.typeVehicle : '643adb7f180420c4805428ea',
        ...
        getUSers() {
            user.getUsers(this.companyId).then((result) => {
                ...
                this.usersOptions = result.data.data.map(function (user) {
                    return {
                        label: user.firstName, value: user._id
                    };
                });
            }).catch((err) => {
        ...
        getType() {
            device.getTypesVehicle().then((result) => {
                this.typeOptions = result.data.data.map(function (type) {
                    return {
                        label: type._id, value: type._id
                    };
                });
            }).catch((err) => {
        ...
        getClases() {
            device.getClassesVehicle().then((result) => {
                this.classesOptions = result.data.data.map(function (classV) {
                    return {
                        label: classV._id, value: classV._id
                    };
                });
            }).catch((err) => {
