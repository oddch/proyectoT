...
<div class="min-h-screen flex flex-col flex-auto flex-shrink-0 antialiased text-gray-800">
  <div class="fixed flex flex-col top-0 left-0 w-60 bg-gray-900 h-full">
    <div class="flex items-center justify-between h-14 border-b px-4 bg-primary-25">
      <RouterLink to="/">
      ...
      <ul class="flex flex-col py-4 space-y-1">        
        ...
        <div v-for="(option, index) in menuOptions" :key="option.name">
          <li class="px-5" v-if="index == (menuOptions.length -1)">
            <div class="flex flex-row items-center h-8">
              <div class="text-sm font-light tracking-wide text-gray-200">Configuración</div>
            </div>
          </li>
          <li>
            <RouterLink 
              :to="option.path" 
              :class="this.$route.path == option.path ? 'text-primary-200 font-semibold bg-primary-25 border-l-4 border-primary-100 hover:text-gray-100' : ''"
            ...
            >
              <span class="inline-flex justify-center items-center ml-4">
                <el-icon size="20" :color="this.$route.path == option.path ? '#3062ff' : '#8FC2D2' ">
              ...
              <span class="ml-2 text-sm tracking-wide truncate"
                :class="this.$route.path == option.path ? 'text-primary-200 font-semibold hover:text-gray-800' : ''"
              >{{ option.name }}</span>
            </RouterLink>
          </li>
        </div>
    ...
    mounted() {
      ...
      if (user != null) {
        ...
        if (user.admin && this.rol == 'Normal') {
          ...
          this.menuOptions = [...this.menuOptions, { path: '/companies', name: 'Compañias' }]
        } else
          if (this.rol == 'Empresa') {
            this.menuOptions = [...this.menuOptions, { path: '/users', name: 'Usuarios' }, { path: '/devices', name: 'Dispositivos' },
            { path: '/travels', name: 'Viajes' }]
          }
        this.menuOptions = [...this.menuOptions, { path: '/login', name: 'Cerrar Sesión' }]
      }
    },
    methods: {
      logout() {
