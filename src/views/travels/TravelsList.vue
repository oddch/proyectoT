                ...
                <tr v-for="travel in travels" :key="travel.travel">
                  ...
                  >
                    ...
                    <el-tooltip
                      class="item"
                      effect="dark"
                      content="Asignar usuario"
                      placement="top-start"
                    >
                      <button
                        @click="openAssignUserModal(travel)"
                        class="ml-4"
                        style="background: none; border: none; cursor: pointer"
                      >
                        <el-icon :size="24" color="#79BDB1">
    ...
    <div v-if="travelToEdit">
      <el-dialog v-model="editTravel" :before-close="closeEditTravelModal">
        <Form
          ...
          :company="travelToEdit"
          nameRef="edit-travel"
        />
      </el-dialog>
      <el-dialog
        v-model="assignUser"
        :before-close="closeEditTravelModal"
        title="Asignar viaje"
      >
        <form-assign-travel
          class="mx-auto w-full"
          :travel="travelToEdit"
          nameRef="assign-travel"
        />
      </el-dialog>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    getTravels() {
      // Get is user from cokies
      let { id } = auth.getUserLogged();
      this.userId = id;
      this.isLoading = true;
      travels
        .getTravelsCompany(this.userId)
        .then((result) => {
          this.travelsList = result.data.data;
    ...
    updateData(newdata) {
      const index = this.travelsList.findIndex(
        (travel) => travel._id === newdata.id
      );

      if (index !== -1) {
        this.travelsList.splice(index, 1, newdata);
      }
