            ...
            <table class="min-w-full text-center text-sm font-light bg-white">
              <thead class="font-mediu text-petroleo-100 bg-gray-100">
                <tr>
                  <th
                    scope="col"
                    class="px-6 py-3 text-left border-tertiary-200"
                  >
                    HABILITADO
                  </th>
                  <th
                ...
                <tr v-for="user in users" :key="user.user">
                  ...
                  >
                    ...
                    <div class="truncate text-left ml-2">
                      ...
                      <p
                        class="text-secondary-300 text-sm text-overline truncate"
                      >
                        {{ user.type }}
                      </p>
                    </div>
                  ...
                  <td
                    class="whitespace-nowrap text-left border-t px-6 py-4 border-tertiary-200"
                  >
                    {{ user.email }}
                  </td>
                  <td
                  ...
                  >
                    {{
                      regions.find(({ value }) => value == user.id_region)
                        ?.label
                    }}
                  </td>
                  <td
                    class="whitespace-nowrap text-left border-t border-tertiary-200 px-6 py-4"
                  >
                    <el-tooltip
                      class="item"
                      effect="dark"
                    ...
                    >
                      <button
                        @click="openEditPasswordUserModal(user)"
                        class="ml-4"
                        style="background: none; border: none; cursor: pointer"
                      >
                        <el-icon :size="24" color="#79BDB1">
...
export default {
  ...
  methods: {
      ...
      companies
        ...
        .then((result) => {
          if (result.data.data != null) {
            this.regions = result.data.data.map(({ nombre_municipio, _id }) => {
              return { label: nombre_municipio, value: _id };
            });
    ...
    updateData(newdata) {
      const index = this.users.findIndex((user) => user._id === newdata.userId);
      if (index !== -1) this.users.splice(index, 1, newdata);
